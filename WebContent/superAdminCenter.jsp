        ...
        <div class="row">
            <div class="col-xs-3" id="myScrollspy">
                ...
                <ul id="main-nav" class="nav nav-tabs nav-stacked" data-spy="affix" data-offset-top="125">
                    ...
                    <li>
                        <a href="#commonSetting" class="nav-header collapsed" data-toggle="collapse">
                            <i class="glyphicon glyphicon-pushpin"></i>
                            本体管理
                            <span class="pull-right glyphicon glyphicon-chevron-toggle"></span>
                        </a>
                        <ul id="commonSetting" class="nav nav-list secondmenu collapse">
                            <li><a href="ontoEditer.jsp"><i class="glyphicon glyphicon-minus"></i>&nbsp;本体编辑</a></li>
                        </ul>
                    </li>

                    <li>
                        <a href="#superSetting" class="nav-header collapsed" data-toggle="collapse">
                            <i class="glyphicon glyphicon-wrench"></i>
                            系统设置（超级管理员）
                            <span class="pull-right glyphicon glyphicon-chevron-toggle"></span>
                        </a>
                        <ul id="superSetting" class="nav nav-list secondmenu collapse">
                            <li><a href="#" onclick="return  modifyParamShowFunction();" id="superSettingShow"><i class="glyphicon glyphicon-minus"></i>&nbsp;修改系统参数</a></li>
                        </ul>
                    </li>
	       ...
	       <!--  ------------------------------------------------------   downloadLogAnalysis  下载日志分析------------------------------------------------------------------------>
            ...
            <!-------------------------------netSearchTimesAnalysisShow 全网搜索信息展示------------------------------- -->
             ...
             <!--- ----------------------------systemParam 系统参数查看与修改------------------------------- -->
              <div id="systemParam" style="display:none" >
                 <h2>系统参数修改</h2>
                 <label>服务器信息</label> <button type="button" class="btn btn-primary" onclick="return openAddServerModal() ;">添加服务器</button>
                 <script id="serverNumTemplate" type="text/html">
					<table class="table table-hover">
						<tr class="success">
							<th class="success">序号</th>
							<th class="success">服务器名字</th>
							<th class="success">ip</th>
							<th class="success">备注</th>
                            <th class="success">操作</th>
                            <th class="success"></th>
						</tr>
						{{each serverList as value i}}
							<tr class="info">
								<td class="success">{{i+1}}</td>
								<td class="success">{{value.serverName}}</td>
								<td class="success"><input type="text" id="{{value.ip}}"class="form-control" placeholder="Text input" value="{{value.ip}}"></td>
								<td class="success">{{value.reMark}}</td>
                                <td class="success"><button type="button" class="btn btn-primary" onclick="return modifyServer('{{value.serverName}}','{{value.ip}}');">保存修改</button></td>
                                 <td class="success"><button type="button" class="btn btn-primary" onclick="return deleteServer('{{value.serverName}}') ;">删除服务器</button></td>
							</tr>
						{{/each}}
					</table>
        		</script>
                <div id="serverNumTableContent"></div>
                <script>
        			refreshServerNumableContent(); 
        			function refreshServerNumableContent()
        			{
        				$.ajax({
        			       type: "POST",
        			       url: 'serverManage.action',
        			       dataType: 'json',
        			       success: function (msg) {
        					var html = template('serverNumTemplate', msg);
        						document.getElementById('serverNumTableContent').innerHTML = html;
        				  	 }
        		   		}); 
        				return false;
        			}	 
                    $.ajax({
        			       type: "POST",
        			       url: 'paramManage.action',
        			       dataType: 'json',
        			       success: function (msg) {
        						document.getElementById('pageMaxSize').value = msg.info.pageMaxSize;
                                document.getElementById('totalHits').value = msg.info.totalHits;
                                document.getElementById('searchMaxItem').value = msg.info.searchMaxItem;
                                document.getElementById('defaultCapacity').value = msg.info.defaultCapacity;
                            	document.getElementById('loginLogMaxSize').value = msg.info.loginLogMaxSize;
                            	document.getElementById('ResourceManagerLogMaxSize').value = msg.info.resourceManagerLogMaxSize;
                            	document.getElementById('downloadName').value = msg.info.downloadName;
                            	document.getElementById('EmailAccountUserName').value = msg.info.emailAccountUserName;
                            	document.getElementById('EmailAccountPassWord').value = msg.info.emailAccountPassWord;
                            	document.getElementById('EmailAccountSmtpHost').value = msg.info.emailAccountSmtpHost;                                                                           
        				  	 }
        		   		}); 
        		</script> 
                <table class="table table-hover">
			     	<tr class="success">
						<th class="success">参数</th>
                        <th class="success">说明</th>
						<th class="success">值</th>
						<th class="success">功能</th>
					</tr>
					<tr class="info">
						<td class="success">pageMaxSize</td>
                        <td class="success">搜索结果每页显示条数</td>
						<td class="success"> <input type="text" id="pageMaxSize" class="form-control" placeholder="Text input"></td>
						<td class="success"><button type="button" class="btn btn-primary" onclick="return saveParam('pageMaxSize');">保存修改</button></td>
				     </tr>
                     <tr class="info">
						<td class="success">totalHits</td>
                        <td class="success">搜索结果只选取条数</td>
						<td class="success"><input type="text" id="totalHits" class="form-control" placeholder="Text input"></td>
						<td class="success"><button type="button" class="btn btn-primary" onclick="return saveParam('totalHits');" >保存修改</button></td>
				     </tr>
                      <tr class="info">
						<td class="success">searchMaxItem</td>
                        <td class="success">服务器最大搜索条数</td>
						<td class="success"> <input type="text" id="searchMaxItem" class="form-control" placeholder="Text input"></td>
						<td class="success"><button type="button" class="btn btn-primary" onclick="return saveParam('searchMaxItem');" >保存修改</button></td>
				     </tr>
                      <tr class="info">
						<td class="success">defaultCapacity</td>
                        <td class="success">注册时网盘默认容量</td>
						<td class="success"> <input type="text" id="defaultCapacity" class="form-control" placeholder="Text input"></td>
						<td class="success"><button type="button" class="btn btn-primary" onclick="return saveParam('defaultCapacity');" >保存修改</button></td>
				     </tr>
                     <tr class="info">
						<td class="success">loginLogMaxSize</td>
                        <td class="success">登录日志默认每页显示条数</td>
						<td class="success"> <input type="text" id="loginLogMaxSize" class="form-control" placeholder="Text input"></td>
						<td class="success"><button type="button" class="btn btn-primary" onclick="return saveParam('loginLogMaxSize');" >保存修改</button></td>
				     </tr>
                     <tr class="info">
						<td class="success">ResourceManagerLogMaxSize</td>
                        <td class="success">资源上传下载默认每页显示条数</td>
						<td class="success"> <input type="text" id="ResourceManagerLogMaxSize" class="form-control" placeholder="Text input"></td>
						<td class="success"><button type="button" class="btn btn-primary" onclick="return saveParam('ResourceManagerLogMaxSize');" >保存修改</button></td>
				     </tr>
                      <tr class="info">
						<td class="success">downloadName</td>
                        <td class="success">服务器下载地址</td>
						<td class="success"> <input type="text" id="downloadName" class="form-control" placeholder="Text input"></td>
						<td class="success"><button type="button" class="btn btn-primary"  onclick="return saveParam('downloadName');" >保存修改</button></td>
				     </tr>
                       <tr class="info">
						<td class="success">EmailAccountUserName</td>
                        <td class="success">邮箱验证账户名</td>
						<td class="success"> <input type="text" id="EmailAccountUserName" class="form-control" placeholder="Text input"></td>
						<td class="success"><button type="button" class="btn btn-primary"  onclick="return saveParam('EmailAccountUserName');"  >保存修改</button></td>
				     </tr>
                       <tr class="info">
						<td class="success">EmailAccountPassWord</td>
                        <td class="success">邮箱验证账户密码</td>
						<td class="success"> <input type="text" id="EmailAccountPassWord" class="form-control" placeholder="Text input"></td>
						<td class="success"><button type="button" class="btn btn-primary" onclick="return saveParam('EmailAccountPassWord');"  >保存修改</button></td>
				     </tr>
                       <tr class="info">
						<td class="success">EmailAccountSmtpHost</td>
                        <td class="success">邮箱验证账户主机地址</td>
						<td class="success"> <input type="text" id="EmailAccountSmtpHost" class="form-control" placeholder="Text input"></td>
						<td class="success"><button type="button" class="btn btn-primary" onclick="return saveParam('EmailAccountSmtpHost');">保存修改</button></td>
				     </tr>
